...
class AI:
    ...
    def is_ship_sunk(self, ship_name, ship_positions, board, ships):
        if all(board[x][y] == self.hit_cell for x, y in ship_positions):
            del ships[ship_name]
            return True
            ...
            if (ai_x, ai_y) not in self.ai_moves and player_board[ai_x][ai_y] not in [hit_cell, miss_cell]:
                ...
                if player_board[ai_x][ai_y] == ship_cell:
                    ...
                    for ship, positions in player_ships.items():
                        if (ai_x, ai_y) in positions and self.is_ship_sunk(ship, positions, player_board, player_ships):
